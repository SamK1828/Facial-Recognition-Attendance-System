...
class Attendance:
    ...
    def __init__(self,root):
        ...
        bg1=bg1.resize((screen_width,screen_width),Image.LANCZOS)
        ...
        #title section
        title_lb1 = Label(bg_img,text="Welcome to Student Attendance Panel",font=("verdana",30,"bold"),bg="white",fg="navyblue")
        ...
        bk_button=Button(title_lb1,command=self.root.destroy,image=self.pback,cursor="hand2")
        bk_button.place(x=1350,y=0,width=120,height=40)
        #========================Section Creating==================================

        # Creating Frame 
        main_frame = Frame(bg_img,bd=2,bg="white") #bd mean border 
        main_frame.place(x=5,y=50,width=screen_width-15,height=screen_height-195)

        # ...........Left Label Frame 
        left_frame = LabelFrame(main_frame,bd=2,bg="white",relief=RIDGE,text="Student Details",font=("verdana",12,"bold"),fg="navyblue")
        left_frame.place(x=5,y=5,width=screen_width//2,height=screen_height-210)
        ...
        time_entry.grid(row=1,column=3,padx=10,pady=10,sticky=W)
        ...
        student_attend_label.grid(row=2,column=2,padx=10,pady=10,sticky=W)
        ...
        attend_combo.grid(row=2,column=3,padx=10,pady=10,sticky=W)

        #.......Table_Frame.......
        table_frame = Frame(left_frame,bd=2,bg="white",relief=RIDGE)
        table_frame.place(x=10,y=150,width=screen_width//2-20,height=screen_height-455)
        ...
        #create table 
        self.attendanceReport_left = ttk.Treeview(table_frame,columns=("ID","Roll_No","Name","Time","Date","Attend"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        # Set Width of Colums 
        self.attendanceReport_left.column("ID",width=50)
        self.attendanceReport_left.column("Roll_No",width=50)
        ...
        self.attendanceReport_left.bind("<ButtonRelease-1>", self.get_cursor_left)

        # =========================Button Section========================

        #Button Frame
        btn_frame = Frame(left_frame,bd=3,bg="skyblue",relief=RIDGE)
        btn_frame.place(x=10,y=480,width=screen_width//2-20,height=60)

        #Import button
        save_btn=Button(btn_frame,text="Import CSV",command=self.importCsv,width=10,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        save_btn.grid(row=0,column=0,padx=10,pady=10,sticky=W)

        #Export button
        update_btn=Button(btn_frame,text="Export CSV",command=self.exportCsv,width=10,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        update_btn.grid(row=0,column=1,padx=10,pady=10,sticky=W)

        #Save button
        del_btn=Button(btn_frame,text="Save",command=self.action,width=10,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        del_btn.grid(row=0,column=2,padx=10,pady=10,sticky=W)

        #Reset button
        reset_btn=Button(btn_frame,text="Reset",command=self.reset_data,width=10,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        reset_btn.grid(row=0,column=3,padx=10,pady=10,sticky=W)

        #Delete Button
        del_btn=Button(btn_frame,text="Delete",width=10,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        del_btn.grid(row=0,column=4,padx=10,pady=10,sticky=W)

        # .................Right Side Section=======================================================

        # Right Label Frame 
        right_frame = LabelFrame(main_frame,bd=2,bg="white",relief=RIDGE,text="Attendance Details",font=("verdana",12,"bold"),fg="navyblue")
        right_frame.place(x=screen_width//20+700,y=5,width=screen_width//2-40,height=screen_height-210)

        # -----------------------------Table Frame-------------------------------------------------
        #Table Frame 
        #Searching System in Right Label Frame 
        table_frame = Frame(right_frame,bd=2,bg="white",relief=RIDGE)
        table_frame.place(x=10,y=90,width=screen_width//2-60,height=screen_height-350)
        ...
        #create table 
        self.attendanceReport = ttk.Treeview(table_frame,columns=("ID","Roll_No","Name","Time","Date","Attend"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        #Update button
        upg_btn=Button(right_frame,command=self.update_data,text="Update",width=12,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        upg_btn.grid(row=0,column=1,padx=6,pady=10,sticky=W)
        #Update button
    ...
    def exportCsv(self):
        try:
            ...
            with open(fln,mode="w",newline="") as myfile:
                ...
                messagebox.showinfo("Successfully","Data Exported Successfully!")
        except Exception as es:
                messagebox.showerror("Error",f"Due to: {str(es)}",parent=self.root)    
